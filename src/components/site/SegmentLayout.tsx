    ...
    <SiteShell>
      <section className="bg-hero">
        <div className="mx-auto max-w-5xl px-4 py-20 sm:px-6 lg:px-8">
          ...
          <h1 className="mt-5 text-balance text-4xl font-semibold leading-tight sm:text-5xl">{spec.title}</h1>
          <p className="mt-4 max-w-3xl text-lg text-muted-foreground">{spec.intro}</p>
          <div className="mt-8 flex flex-wrap gap-3">
            <Button asChild size="lg"><Link to={spec.primaryCta.to}>{spec.primaryCta.label}<ArrowRight className="ml-2 h-4 w-4" /></Link></Button>
            {spec.secondaryCta && <Button asChild size="lg" variant="outline"><Link to={spec.secondaryCta.to}>{spec.secondaryCta.label}</Link></Button>}
          </div>
        </div>
      </section>
      ...
      {spec.notice && (
        <section className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
          <div className="rounded-2xl border border-border bg-muted/40 p-5 text-sm text-muted-foreground">{spec.notice}</div>
        </section>
      )}
