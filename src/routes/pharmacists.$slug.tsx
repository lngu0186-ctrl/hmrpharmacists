...
export const Route = createFileRoute("/pharmacists/$slug")({
  ...
  head: ({ params }) => ({
    meta: [
      ...
      {
        name: "description",
        content: "Verified credentialed pharmacist profile for Home Medicines Reviews.",
      },
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("pharmacists")
        .select(
          "*, pharmacist_specialties(specialty), pharmacist_languages(language), pharmacist_service_areas(suburb,state,postcode,radius_km), pharmacist_affiliations(organisation,role)",
        )
        .eq("slug", slug)
            ...
            <div className="rounded-3xl border border-border bg-card p-6 shadow-soft sm:p-8">
              <div className="flex flex-col gap-5 sm:flex-row sm:items-start">
                <img
                  ...
                  alt={p.full_name}
                  className="h-24 w-24 rounded-2xl object-cover ring-2 ring-primary-soft"
                />
                <div className="flex-1">
                  ...
                  <div className="mt-3 flex flex-wrap gap-x-5 gap-y-1.5 text-sm text-muted-foreground">
                    <span className="inline-flex items-center gap-1">
                      <MapPin className="h-3.5 w-3.5" /> {p.suburb}, {p.state} {p.postcode}
                    </span>
                    {p.years_experience && (
                      <span className="inline-flex items-center gap-1">
                        <Award className="h-3.5 w-3.5" /> {p.years_experience} years experience
                      </span>
                    )}
                    {p.turnaround_days && (
                      <span className="inline-flex items-center gap-1">
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-3">
                ...
                <Capability
                  label="Home visits"
                  enabled={p.home_visits}
                  icon={<Home className="h-4 w-4" />}
                />
                <Capability
                  label="Accepting referrals"
                  enabled={p.accepting_referrals}
              ...
              <div className="mt-6 rounded-xl border border-border bg-muted/40 p-4 text-xs text-muted-foreground">
                <strong className="text-foreground">Verification:</strong> AHPRA registration and
                credentialing evidence reviewed manually by our team. Listing on this platform does
                not guarantee referral acceptance.
              </div>
            </div>
          </div>
          ...
          <aside>
            <div className="sticky top-20 rounded-3xl border border-border bg-card p-6 shadow-soft">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Privacy-safe. Your contact details go to {p.full_name.split(" ")[0]} via the
                platform.
              </p>
...
function Capability({
  ...
  return (
    <div
      className={`flex items-center gap-2 rounded-xl border px-3 py-2.5 ${enabled ? "border-success/30 bg-success/5 text-success" : "border-border bg-muted/40 text-muted-foreground"}`}
    >
      {icon}
    ...
    >
      <div>
        ...
        <Select
          ...
          onValueChange={(v) => setForm({ ...form, sender_type: v as typeof form.sender_type })}
        >
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <Field
          ...
          onChange={(v) => setForm({ ...form, sender_phone: v })}
        />
        <Field
      ...
      <Field
        label="Patient suburb (optional)"
        value={form.patient_suburb}
        onChange={(v) => setForm({ ...form, patient_suburb: v })}
      />
      <div>
        ...
        <Textarea
          ...
          value={form.message}
          placeholder="Briefly describe what you need — please do not include sensitive clinical detail or full medication lists."
          onChange={(e) => setForm({ ...form, message: e.target.value })}
          required
          maxLength={1500}
        />
        <p className="mt-1 text-[11px] text-muted-foreground">
          Do not include diagnoses, medication lists or other sensitive health data.
        </p>
      </div>
      <label className="flex items-start gap-2.5 rounded-lg border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
        <Checkbox
          ...
          onCheckedChange={(v) => setForm({ ...form, consent_given: !!v })}
          className="mt-0.5"
        />
        <span>
          I consent to my contact details being shared with the listed pharmacist via this platform
          for the purpose of this enquiry. I understand this is not medical advice or emergency
          care.
        </span>
      </label>
