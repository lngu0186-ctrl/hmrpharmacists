...
export const Route = createFileRoute("/_authenticated/onboarding")({
  ...
  head: () => ({
    meta: [
      { title: "Pharmacist onboarding — HMR Pharmacist Exchange" },
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (form.specialties.length) {
        await supabase
          .from("pharmacist_specialties")
          .insert(form.specialties.map((s) => ({ pharmacist_id: pharmacistId!, specialty: s })));
      }
      // Service area
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">
          {existing ? "Edit your profile" : "Pharmacist onboarding"}
        </h1>
      ...
      <Card className="p-6 sm:p-8">
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            ...
            <Field label="AHPRA number *">
              <Input
                ...
                onChange={(e) => update("ahpra_number", e.target.value)}
            ...
            <Field label="Credentialing body">
              <Input
                ...
                onChange={(e) => update("credentialing_body", e.target.value)}
        ...
        {step === 1 && (
          <div className="grid gap-5 sm:grid-cols-3">
            ...
            <div className="sm:col-span-3 grid gap-3 sm:grid-cols-3">
              <Toggle
                label="Home visits"
                value={form.home_visits}
                onChange={(v) => update("home_visits", v)}
              />
              <Toggle
              ...
              />
              <Toggle
                label="Currently accepting referrals"
                value={form.accepting_referrals}
                onChange={(v) => update("accepting_referrals", v)}
              />
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div>
            <p className="mb-4 text-sm text-muted-foreground">
              Select all areas of clinical focus that apply.
            </p>
        ...
        {step === 3 && (
          <div className="grid gap-5">
            <Field label="Typical turnaround (days)">
              <Input
                type="number"
                value={form.turnaround_days ?? 7}
                onChange={(e) => update("turnaround_days", Number(e.target.value))}
              />
            </Field>
            <Field label="Preferred contact method">
              <Input
                ...
                placeholder="Email / Secure messaging"
              />
            </Field>
            <Field label="Bio (visible on your public profile)">
              <Textarea
                ...
                placeholder="Briefly describe your practice, approach, and the patients you typically support…"
              />
              <p className="mt-1 text-[11px] text-muted-foreground">
                {(form.bio ?? "").length} / 2000 characters. Avoid clinical claims you cannot
                substantiate.
              </p>
            </Field>
          </div>
        ...
        {step === 4 && (
          <div className="space-y-4">
            ...
            <p className="text-xs text-muted-foreground">
              Submitting sends your profile for admin verification. We confirm AHPRA registration
              and credentialing before publishing publicly. You can edit any time.
            </p>
          </div>
        )}
        ...
        <div className="mt-8 flex items-center justify-between border-t border-border pt-6">
          ...
          {step < steps.length - 1 ? (
            ...
            <Button onClick={submit} disabled={saving}>
              {saving ? "Saving…" : existing ? "Save changes" : "Submit for verification"}
            </Button>
          )}
        </div>
      </Card>
