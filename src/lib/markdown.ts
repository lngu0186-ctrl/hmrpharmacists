...
export function renderMarkdown(md: string): string {
  ...
  for (const raw of lines) {
    ...
    if ((m = line.match(/^###\s+(.+)/))) {
      closeList();
      out.push(`<h3 class="mt-6 text-lg font-semibold">${inline(m[1])}</h3>`);
    } else if ((m = line.match(/^##\s+(.+)/))) {
      closeList();
      out.push(`<h2 class="mt-8 text-2xl font-semibold tracking-tight">${inline(m[1])}</h2>`);
    } else if ((m = line.match(/^#\s+(.+)/))) {
      closeList();
      out.push(
        `<h1 class="mt-2 text-3xl font-semibold tracking-tight sm:text-4xl">${inline(m[1])}</h1>`,
      );
    } else if ((m = line.match(/^[-*]\s+(.+)/))) {
      if (!inList) {
        out.push('<ul class="my-4 ml-6 list-disc space-y-1.5 text-muted-foreground">');
        inList = true;
      }
      out.push(`<li>${inline(m[1])}</li>`);
    } else {
      closeList();
      out.push(`<p class="my-4 leading-relaxed text-muted-foreground">${inline(line)}</p>`);
    }
  }
  closeList();
  return out.join("\n");
